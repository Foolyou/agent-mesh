...
- **Cache** key `(mesh, owner, file, size, mtime) → image_key` (only successful uploads cached). **Limits**: > `maxBytes` (default 10MB; artifacts are already ≤5MB) or aspect `height:width > 16:9` → degrade. **Degrade** = lark_md link to the device-auth console artifact URL when `MESH_CONSOLE_URL` is set, else plain text. **Secret hygiene**: never logs the raw ref / bytes / image_key; the placeholder + degrade markdown carry only the human `alt`.
