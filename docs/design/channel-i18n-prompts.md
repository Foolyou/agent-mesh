...
prompt scaffolding → English **structured-instruction** style. Mirrored user/agent conversation content
...
the Feishu user, and is distinct from Category A. It must be fully converted to a concise English
**structured-instruction** style (NOT `[REQ]/[FYI]/[DONE]` notifications) — fields like
`source` / `chat_type` / `instructions` / `user_message`.
...
### Category C (channel→agent, approved) — structured-instruction style (NOT mail-tagged)

C frames read like concise agent instructions with structured `key: value` fields (no `[FYI]` tag). The
`user_message` value is the mirrored user text (Category D) — never altered; only the frame is English.
...
- **C3 — Category C + card fallback + team3 tool integration**:
  1. Convert the channel→agent frames — `feishuUserPrompt` (`feishu.prompt.group`) and
     `feishuAssistantPrompt` (`feishu.prompt.p2p`) — to the structured-instruction style, and the
...
- **Category C prompt snapshots** (C3): assert the structured-instruction frame produced for a group
...
## Decisions (confirmed by prdmgr/user) + remaining taste items

Confirmed:
1. **Category C** (channel→agent injected prompt scaffolding): **APPROVED, first-class, in scope** —
   convert to the English structured-instruction style (NOT mail-tagged), in C3.
...
Remaining taste items for review (Category A wording was already accepted; flag any change):
- Mail-tag taxonomy (errors `[FYI]`, auth-required `[REQ]`, completed `[DONE]`) and the `status: running/
  stopped` lines — already approved; re-confirm if the C-promotion changes any preference.
- Category C field set: `source` / `chat_type` / `role` (p2p only) / `instructions` / `user_message` —
  confirm the field names/order and the image payload wording `[the user sent an image]`.
