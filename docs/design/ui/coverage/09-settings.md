...
- 2026-06-22 — Step 7.4-B `/bnw/settings` real wiring (`src/web/client/bnw/settings.tsx` + `bnw/prefs.ts` + `clearActive()` added to themes.ts; old Theme.tsx/ThemePicker untouched). Appearance: mode×accent SegmentedControls + a REAL 9-combo live preview grid (each cell = `compose(m,a)` colors, clickable to apply) + custom-palette editor — all call the real `applyComposition(compose())` / `applyPalette()` path and write `:root`, persisting `saveMode`/`saveAccent` (+`clearActive`) or `saveCustomPalette`+`saveActive("custom")`. Language: real `saveLang` (mesh.lang + `<html lang>`). Flagged deviations: (a) the mockup stacks all groups; the locked routing requires `?tab=appearance|language|prefs|devices`, so groups are rendered as URL-addressable tabs; (b) preferences (default-view/default-device) persist CLIENT-LOCAL (localStorage `mesh.bnw.*`), explicitly NOT a server write — default-view is honored by BnwApp's landing redirect, default-device is inert until a manual layout override lands (Step 7.5); (c) device management is an HONEST placeholder — only the own-device status is read (pollDeviceStatus); list/approve/revoke/bootstrap stay host-CLI authoritative (`mesh device list|approve|revoke`, `mesh auth bootstrap`) — no web seam exists (audited: api.ts has only device start/status/verify/bootstrap; revoke/list are CLI-only), same posture as channels Option B; (d) /bnw view strings are not yet fully i18n-keyed — language selection persists + drives `<html lang>`/old console, with full /bnw string i18n a separate pass.
