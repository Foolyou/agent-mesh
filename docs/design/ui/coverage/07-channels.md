...
- 2026-06-22 — Step 7.4-A.2b-i `/bnw/channels` real wiring (security-driven deviation, prdmgr-approved **Option B**): the real surface wires only the EXISTING safe web APIs — `getFeishuStatus` (ChannelStatusCard) and `startFeishuProvision`+poll/cancel / `syncFeishuMeshChats` / `ensureFeishuMeshChat` (ChannelBindingsCard: bindings + sync + per-mesh ensure-group + provision QR/verify/expiry/cancel). The mockup-07 **PendingSenders (authcode approve/revoke) and AuthorizedSenders (allowSenders revoke)** sections are rendered as **explicit placeholders only** (no fake actions): sender authorization is host-CLI authoritative (`mesh channels feishu list | approve <code> | revoke <channelKey> <openId>`), and a web admin seam over auth-store is deferred to the device-auth/auth-store slice. Rationale: full PendingSenders/AllowSenders parity would create a new security-sensitive web admin API over auth-store while current authority is the host CLI — a product/security decision, not a builder-local choice. Real `/bnw` view (`src/web/client/bnw/channels.tsx`); old FeishuPanel untouched.
