...
- 2026-06-22 — Step 7.4-A.2b-ii `/bnw/mesh/<id>/agent/<id>/file|artifact/<path>` real wiring (`src/web/client/bnw/file-viewer.tsx`, independent of the old FileViewer): Bearer fetch (authHeaders) → rendered markdown (shared Markdown + AuthorContext) / plain code / image; URL-addressable lightbox (`?lb=1`) with close + Esc; back→runtime focus; loading / 404 / not-permitted(401) states. Flagged deviations from mockup 11: (a) code renders as a plain mono `<pre>` — the v2 design defines no legacy `tok-*` syntax-color classes, so the old syntax highlighter is dropped (can return as a v2-token pass); (b) the composer pending-image tray bundled into mockup 11 is a runtime-focus/composer concern, not part of the standalone deep-linked artifact route, so it is not rendered on this surface.
