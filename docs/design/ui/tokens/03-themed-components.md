...
- `v2-sample-dark-slate.png` · `v2-sample-light-cool.png` · `v2-sample-eye-care-warm.png` — per-mode component sample boards (1480px; **re-rendered for v2.1**), each showing all 3 accents (Signal Teal / Ember / Fleet Azure) across StatusChip · Button states (danger = `danger` fill + `on-danger`) · **ErrorBanner (`danger-subtle` bg)** · StatusListRow (incl. selected = `accent-subtle`) · IssueListRow (progress = `accent`) · Composer (send = `accent`/`on-accent`) · ApprovalCard (deny = `danger` fill + `on-danger`).
- `v2-status-tokens.png` (1200px) — **v2.1 status tokens**: per mode, the 4 statuses as `filled` (status fill + `on-<status>`) beside `<status>-subtle`; shows `on-*` per-mode resolution (Dark near-black on bright fills incl. amber warning; Light/Eye-care white on dark fills).
...
- 2026-06-20 — **v2.1 status tokens**: ErrorBanner now references `danger-subtle` (named token, not a component-side 12% formula); StatusChip filled/soft variants, Badge, Button danger, Composer interrupt, ApprovalCard deny use `on-<status>` for filled-status text.
- 2026-06-20 — **v2.1 artifacts**: per-mode sample boards re-rendered (`on-danger` on danger Button + ApprovalCard deny; ErrorBanner on `danger-subtle`); added `v2-status-tokens.png` (filled `on-<status>` + `<status>-subtle` per mode).
