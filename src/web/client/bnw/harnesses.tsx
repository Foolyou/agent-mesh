...
function statusLabel(row: HarnessProbeRow, t: TFn): { text: string; kind: HarnessKind } {
  if (!row.installed) return { text: t("bnw.hn.statusMissing"), kind: "bad" };
  if (row.error === "registry-unavailable" || !row.latest) return { text: t("bnw.hn.statusNoCompare"), kind: "off" };
  if (row.outdated && row.version) return { text: t("bnw.hn.statusUpdate", { v: row.version, latest: row.latest }), kind: "warn" };
  if (row.version) return { text: t("bnw.hn.statusInstalled", { v: row.version }), kind: "ok" };
  return { text: t("bnw.hn.statusUnknown"), kind: "off" };
}
...
export function BnwHarnesses({ store }: { store: Store }) {
  ...
  function applyInstallEvent(event: HarnessInstallEvent, label: string) {
    ...
    setInstall((cur) => {
      ...
      if (event.step === "done") return { ...cur, status: "done", liveText: t("bnw.hn.installed", { x: event.installedVersion ? `v${event.installedVersion}` : label }), lines: nextLines };
      if (event.step === "error") return { ...cur, status: "error", liveText: event.message ?? t("bnw.hn.installFailed"), lines: nextLines };
  ...
  async function startInstall(row: HarnessProbeRow) {
    try {
      ...
      setInstall({ harness: row.id, pkgSpec: job.pkgSpec, status: "running", liveText: t("bnw.hn.installing", { label: row.label }), lines: [] });
      await store.streamHarnessInstall(row.id, job.jobId, (e) => applyInstallEvent(e, row.label), (err) => {
        if (err) setInstall((cur) => (cur && cur.status === "running" ? { ...cur, status: "interrupted", liveText: t("bnw.hn.streamInterrupted") } : cur));
      }).catch(() => {});
  ...
  const actions = <Button size="sm" variant="ghost" disabled={offline} aria-label="refresh harness status" onClick={() => void load()}>{t("bnw.refresh")}</Button>;
            ...
            <div className="flex flex-col rounded-lg border border-border bg-surface-raised">
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="border-b border-border px-3 py-2.5 last:border-0">
                  <div className="flex items-center gap-2"><StatusChip status="idle" variant="dot" /><span className="text-sm text-text-muted">{t("bnw.hn.loadingStatus")}</span></div>
                  <div className="mt-1"><Skeleton variant="line" /></div>
                </div>
              ))}
            </div>
          ) : phase === "error" ? (
            <ErrorBanner title={t("bnw.hn.probeFailed")} onRetry={() => void load()}>{t("bnw.hn.probeFailedDesc")}</ErrorBanner>
          ) : (
            <>
              <div className="flex flex-col rounded-lg border border-border bg-surface-raised">
...
export function HarnessRow({ row, disabled = false, onReprobe, onInstall }: { row: HarnessProbeRow; disabled?: boolean; onReprobe: () => void; onInstall: () => void }) {
  ...
  const line = row.installed ? harnessVersionLine(row) : (self?.docsUrl ?? row.path ?? t("bnw.hn.notDetected"));
  ...
  return (
    <div data-harness-row className="flex flex-col gap-1 border-b border-border px-3 py-2.5 last:border-0">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {row.auth === "required" ? <StatusChip status="attention" variant="soft" label={t("bnw.hn.authRequired")} /> : null}
        ...
        {self ? null : <Button size="sm" variant="ghost" disabled={disabled} aria-label={`reprobe ${row.id}`} className="whitespace-nowrap" onClick={onReprobe}>{t("bnw.hn.reprobe")}</Button>}
        {canInstall ? <Button size="sm" variant="secondary" disabled={disabled || row.installable !== "npm"} aria-label={`${status.kind === "warn" ? "update" : "install"} ${row.id}`} className="whitespace-nowrap" onClick={onInstall}>{status.kind === "warn" ? t("bnw.hn.update") : t("bnw.hn.install")}</Button> : null}
      </div>
...
export function SelfInstallerGuide({ id, command, docsUrl, disabled = false, onReprobe }: { id: string; command: string; docsUrl: string; disabled?: boolean; onReprobe: () => void }) {
  ...
  return (
    <div data-self-installer className="mt-1.5 flex flex-wrap items-center gap-2 rounded-lg border border-border bg-surface-sunken px-2.5 py-1.5 text-xs">
      <span className="text-text-muted">{t("bnw.hn.selfInstall")}</span>
      ...
      <Button size="sm" variant="ghost" disabled={disabled} aria-label={`copy install command for ${id}`} className="whitespace-nowrap"
        onClick={async () => { try { await navigator.clipboard?.writeText(command); setCopied(true); setTimeout(() => setCopied(false), 1600); } catch { /* clipboard unavailable */ } }}>{copied ? t("bnw.hn.copied") : t("bnw.hn.copyCommand")}</Button>
      <a className="rounded-lg px-2 py-1 text-link underline-offset-2 hover:underline" href={docsUrl} target="_blank" rel="noreferrer" aria-label={`open ${id} docs`}>{t("bnw.hn.docs")} ↗</a>
      <Button size="sm" variant="ghost" disabled={disabled} aria-label={`reprobe to detect ${id}`} className="whitespace-nowrap" onClick={onReprobe}>{t("bnw.hn.reprobeDetect")}</Button>
    </div>
  );
}
  ...
  return (
    <div data-install-progress className="flex flex-col gap-2 rounded-lg border border-border bg-surface-sunken p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {install.status === "interrupted" ? <Button size="sm" variant="primary" aria-label="retry stream" onClick={onRetry}>{t("bnw.hn.retryStream")}</Button> : null}
        {install.status !== "running" ? <Button size="sm" variant="ghost" aria-label="close install progress" onClick={onClose}>{t("bnw.close")}</Button> : null}
      </div>
...
export function OldVersionAgentsCard({ agents, store, disabled = false }: { agents: OldAgentEntry[]; store: Store; disabled?: boolean }) {
  ...
  return (
    <div data-old-agents className="flex flex-col gap-2 rounded-lg border border-border bg-surface-raised p-3">
      <span className="text-xs uppercase tracking-wider text-text-muted">{t("bnw.hn.oldAgents", { n: agents.length })}</span>
        ...
        {agents.map((a) => {
          ...
          return (
            <div key={a.entry} data-old-agent className="flex flex-wrap items-center gap-2 border-b border-border pb-1.5 text-sm last:border-0">
              ...
              <span className="font-mono text-xs text-text-muted">{t("bnw.hn.runningOlder", { h: a.harnessId })}{a.target ? ` → v${a.target}` : ""}</span>
              <span className="flex-1" aria-hidden="true" />
              {isPending ? (
                <>
                  <span className="text-xs text-text-muted">{t("bnw.hn.restartPending")}</span>
                  <Button size="sm" variant="ghost" disabled={disabled} aria-label={`cancel restart ${a.entry}`} className="whitespace-nowrap" onClick={() => respawn(a.entry, "cancel")}>{t("cancel")}</Button>
                </>
              ) : (
                <>
                  <Button size="sm" variant="secondary" disabled={disabled} aria-label={`restart ${a.entry} after idle`} className="whitespace-nowrap" onClick={() => respawn(a.entry, "after-idle")}>{t("bnw.hn.afterTurn")}</Button>
                  <ConfirmButton size="sm" variant="danger" disabled={disabled} aria-label={`force restart ${a.entry}`} confirmLabel={t("bnw.hn.forceConfirm")} className="whitespace-nowrap" onConfirm={() => respawn(a.entry, "force")}>{t("bnw.hn.force")}</ConfirmButton>
                </>
              )}
            </div>
          );
