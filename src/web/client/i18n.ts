...
const DICT: Record<string, Entry> = {
  ...
  bypass: ["bypass", "绕过权限"],
  "bypass.hint": ["permission bypass — saved now and applied when supported", "绕过权限 —— 立即保存；支持时生效"],
  "bypass.hint.live": ["permission bypass is spawn-time only for this harness — stop the mesh to change", "该 harness 仅启动时应用绕过权限 —— 停止 mesh 后可修改"],
  "bypass.hint.runtime": ["runtime bypass maps this Codex agent to full-access mode", "运行时绕过权限会把此 Codex agent 切到 full-access 模式"],
  "bypass.hint.unsupported": ["permission bypass is not supported by this harness", "该 harness 不支持绕过权限"],
