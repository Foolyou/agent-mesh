...
await writeFile(join(work, "long.md"), `# Long Report\n\n${Array.from({ length: 220 }, (_, i) => `Line ${i + 1}: scrollable markdown content.`).join("\n\n")}\n`);
...
try {
  ...
  await step("mobile: long Markdown scrolls inside the file viewer body", async () => {
    ...
    await page.goto(`${BASE}/mesh/files/agent/codex-1/file/long.md`, { waitUntil: "domcontentloaded" });
    await page.waitForSelector('.file-viewer-body h1:has-text("Long Report")', { timeout: 8000 });
    const initialScrollTop = await page.locator(".file-viewer-body").evaluate((el) => el.scrollTop);
    if (initialScrollTop !== 0) throw new Error(`initial scrollTop ${initialScrollTop} !== 0`);
    await page.locator(".file-viewer-body").evaluate((el) => el.scrollBy(0, 600));
    const metrics = await page.locator(".file-viewer-body").evaluate((el) => ({
      scrollTop: el.scrollTop,
      scrollHeight: el.scrollHeight,
      clientHeight: el.clientHeight,
    }));
    if (metrics.scrollTop <= 0) throw new Error(`scrollTop did not move: ${JSON.stringify(metrics)}`);
    if (metrics.scrollHeight <= metrics.clientHeight) throw new Error(`body is not overflowing: ${JSON.stringify(metrics)}`);
  });
