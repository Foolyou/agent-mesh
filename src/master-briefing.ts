...
export function buildMasterBriefing(): string {
  return [
    ...
    "- Each agent spec may include agents[].instructions: optional per-agent instructions injected only into that agent's briefing.",
    "- Use create_mesh to set per-agent instructions for new meshes.",
    "- To modify or clear per-agent instructions on an existing mesh, use get_mesh, edit the relevant agents[].instructions values in the complete spec, then call update_mesh while the mesh is stopped.",
    "- Use the mesh-wide charter only for shared team goals and norms that every agent should see; do not put agent-private guidance in charter when agents[].instructions fits.",
    "",
