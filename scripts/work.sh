...
BASE="${MESH_WORK_ROOT:-$HOME}"               # base dir; data lives in <base>/.agent-mesh
PORT="${MESH_WORK_PORT:-10010}"
[ -x ./dist/mesh ] || { echo "dist/mesh missing — run: bun run build" >&2; exit 1; }
echo "production agent-mesh → http://localhost:$PORT  (root $BASE/.agent-mesh, pinned binary)"
exec ./dist/mesh --port "$PORT" --root "$BASE"
